 ...
 * @LastEditTime: 2022-03-23 15:42:29
 * @FilePath: \webpack-teste:\others\jsplumb-test\src\views\antv\x6Panel\NodeCom.vue
-->
  ...
  <div class="node-wrap">
    <div class="node-content">
      ...
      <template v-if="nodeInfo.type !== 'zb' && !nodeInfo.isLeaf">
        <span class="node-icon icon-right" @click="handleClick('right')"
          >下{{rightVisible ? '+' : '-'}}</span
        >
        <span class="node-icon icon-bottom" @click="handleClick('bottom')"
          >指{{bottomVisible ? '+' : '-'}}</span
        >
      </template>
